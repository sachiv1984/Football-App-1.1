...
const Badge: React.FC<BadgeProps> = ({
  ...
  const solidVariantClasses: Record<BadgeProps['variant'], string> = {
    ...
    danger: 'bg-red-600 text-white', // added danger
  };

  const outlineVariantClasses: Record<BadgeProps['variant'], string> = {
    ...
    danger: 'border border-red-600 text-red-700 bg-red-50', // added danger
  };

  const variantClasses = outline ? outlineVariantClasses[variant] : solidVariantClasses[variant];

  const badgeClasses = clsx(baseClasses, sizeClasses[size], variantClasses, {
