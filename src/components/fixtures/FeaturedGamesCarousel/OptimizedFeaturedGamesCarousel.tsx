...
interface Props {
  fixtures?: FeaturedFixtureWithImportance[]; // optional for hybrid behavior
...
const OptimizedFeaturedGamesCarousel: React.FC<Props> = ({
  ...
  const { featuredFixtures: apiFixtures, loading, error } = useFixtures();
  const dataToRender = fixtures ?? apiFixtures;
  ...
  const totalSlides = dataToRender.length;
    ...
    <div className={clsx('carousel-apple', className)} role="region" aria-label="Featured Games Carousel">
      ...
      >
        <div ref={trackRef} className="carousel-track">
          {dataToRender.map((fixture, index) => {
            ...
            return (
              <div key={fixture.id || index} className="carousel-slide" aria-hidden={currentSlide !== index}>
                <div
                  ...
                  role="button"
                >
                  {/* Render your fixture content here as before */}
                </div>
              </div>
            );
          })}
