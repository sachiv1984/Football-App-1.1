...
const OptimizedFeaturedGamesCarousel: React.FC<FeaturedGamesCarouselProps> = ({
  fixtures = [],
  onGameSelect = (fixture: FeaturedFixtureWithImportance) => console.log('Selected fixture:', fixture.id),
  ...
  const {
    ...
    refreshData,
    scrollRef
  } = useFeaturedGamesCarousel({ fixtures, config: { selection: selectionConfig }, autoRefresh: true, rotateInterval });

  useEffect(() => {
    if (autoRotate && !carouselState.isAutoRotating) toggleAutoRotate();
  }, [autoRotate, carouselState.isAutoRotating, toggleAutoRotate]);
    ...
    return (
      <section className={`bg-gradient-hero text-white section-sm ${className}`}>
        <div className="container text-center py-12">
          <Trophy className="w-12 h-12 mx-auto mb-2 text-red-300" />
      ...
      <div className="container">
        ...
        <div className="flex items-center justify-between mb-6 animate-fade-in">
          <div className="flex items-center">
            <Trophy className="w-6 h-6 text-electric-yellow mr-3" />
            <h2 className="text-2xl font-bold">Featured Games</h2>
            {featuredGames.some(f => f.status === 'live') && (
              ...
              {featuredGames.map((_, idx) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-all ${
                    idx === carouselState.currentIndex ? 'bg-electric-yellow' : 'bg-white/30'
                  }`}
          ...
          {featuredGames.map(fixture => {
            ...
            return (
              <div
                ...
                role="button"
                tabIndex={0}
                onKeyDown={e => { if (['Enter',' '].includes(e.key)) { e.preventDefault(); onGameSelect(fixture); } }}
              ...
              >
                ...
                <div className="relative">
                  {isLive ? (
                    ...
                    <div className="bg-electric-yellow text-gray-900 text-xs font-bold px-3 py-1 rounded-t-xl flex items-center">
                      <Trophy className="w-3 h-3 mr-1" />BIG MATCH
                    </div>
                  ) : null}
                </div>
                ...
                <div className="p-5 text-gray-900">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center">
                      <img src={fixture.competition?.logo} alt={fixture.competition?.name} className="w-5 h-5 rounded mr-2" loading="lazy"/>
                      <span className="text-sm font-medium text-gray-600">{fixture.competition?.shortName ?? 'Comp'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="badge badge-sm badge-secondary">Week {fixture.matchWeek ?? 'TBD'}</span>
                      {fixture.importanceScore && fixture.importanceScore >= 8 && <span className="badge badge-sm badge-warning">🔥 Hot</span>}
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center flex-1">
                        <img src={fixture.homeTeam?.logo} alt={fixture.homeTeam?.name} className="team-logo mr-3" loading="lazy"/>
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-sm truncate">{fixture.homeTeam?.shortName ?? 'Home'}</div>
                          <div className="text-xs text-gray-500">#{fixture.homeTeam?.position ?? '-'}</div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-center py-2">
                      {isLive ? (
                        <div className="text-center">
                          <div className="text-2xl font-bold text-gray-900 font-mono">{fixture.homeScore ?? 0} - {fixture.awayScore ?? 0}</div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center flex-1">
                        <img src={fixture.awayTeam?.logo} alt={fixture.awayTeam?.name} className="team-logo mr-3" loading="lazy"/>
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-sm truncate">{fixture.awayTeam?.shortName ?? 'Away'}</div>
                          <div className="text-xs text-gray-500">#{fixture.awayTeam?.position ?? '-'}</div>
                        </div>
                      </div>
                  ...
                  <div className="space-y-2 text-xs text-gray-600 mb-4">
                    ...
                    {!isLive && <div className="flex items-center"><Clock className="w-3 h-3 mr-2 flex-shrink-0"/><span>{time}</span></div>}
                    <div className="flex items-center"><MapPin className="w-3 h-3 mr-2 flex-shrink-0"/><span className="truncate">{fixture.venue}</span></div>
                  </div>
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <h4 className="text-xs font-semibold text-teal-900 truncate">{fixture.aiInsight.title}</h4>
                            <span className="text-xs font-medium text-teal-700">{fixture.aiInsight.confidence}</span>
                          </div>
                          <p className="text-xs text-teal-800 truncate">{fixture.aiInsight.summary}</p>
                        </div>
                      </div>
                    </div>
                  )}
