...
const OptimizedFeaturedGamesCarousel: React.FC<Props> = ({
  ...
  const getLogo = (team: any) =>
    team?.crest || (team?.shortName ? `/team-logos/${team.shortName.replace(/\s+/g, "-").toLowerCase()}.png` : "/team-logos/default.png");

  // 1-3-1 pattern: compute indices to render
  const visibleIndices = [
    (currentIndex - 1 + totalFixtures) % totalFixtures,
    currentIndex,
    ...
    >
      ...
      <div className="relative h-[360px] sm:h-[400px]">
          ...
          {visibleIndices.map((idx) => {
            ...
            return (
              <motion.div
                ...
                exit={{ opacity: 0, x: idx < currentIndex ? 100 : -100 }}
                    ...
                    {fixture.tags?.length ? (
                      <div className="flex flex-wrap justify-center gap-2 mt-2">
                        {fixture.tags.map((tag, idx) => (
                          <span key={idx} className="px-2 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-700 border border-purple-200">{tag}</span>
                        ))}
                      </div>
                    ) : null}
                  </div>
        ...
        <div className="absolute inset-y-0 left-0 flex items-center pl-2 sm:pl-4">
          <button onClick={goToPrevious} className="hidden sm:flex items-center justify-center w-10 h-10 rounded-full bg-purple-800 bg-opacity-60 text-white hover:bg-opacity-90 transition">
            <ChevronLeft className="w-6 h-6" />
          </button>
        </div>
        <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:pr-4">
          <button onClick={goToNext} className="hidden sm:flex items-center justify-center w-10 h-10 rounded-full bg-purple-800 bg-opacity-60 text-white hover:bg-opacity-90 transition">
      ...
      <div className="flex justify-center space-x-2 pb-4 mt-2">
        {fixtures.map((_, idx) => (
          <button key={idx} className={`w-2.5 h-2.5 rounded-full transition ${idx === currentIndex ? "bg-purple-600 w-6" : "bg-purple-300 hover:bg-purple-400"}`} onClick={() => setCurrentIndex(idx)} />
        ))}
      </div>
    </div>
  );
