...
export class FixtureService {
  private readonly CACHE_KEY = 'matches:pl';
  private readonly ETAG_KEY = 'matches:pl:etag';
  ...
  // -------------------------
  // Utility: measure async function timing
  // -------------------------
  ...
  private async getTeamDetails(
    team: RawMatch['homeTeam'] | RawMatch['awayTeam'],
    standings: RawStanding[]
  ): Promise<TeamWithForm> {
  ...
  // Redis caching
  // -------------------------
  private async getCachedMatches(): Promise<FeaturedFixtureWithImportance[] | null> {
    return redisGet<FeaturedFixtureWithImportance[]>(this.CACHE_KEY);
  }

  private async setCachedMatches(matches: FeaturedFixtureWithImportance[], ttlSeconds: number): Promise<void> {
    await redisSet(this.CACHE_KEY, matches, ttlSeconds);
  }
  ...
  private async refreshCache(): Promise<void> {
    ...
    // Determine TTL: live -> 30s, matchday -> 15min, default 10min
    let ttl = 10 * 60; // 10 min default
    const hasLive = transformed.some(m => m.status === 'live');
    if (hasLive) ttl = 30;
    else {
      const today = new Date();
      const hasToday = transformed.some(m => new Date(m.dateTime).toDateString() === today.toDateString());
      if (hasToday) ttl = 15 * 60; // 15 min matchday
    }

    await this.setCachedMatches(transformed, ttl);
  }

  private async ensureCache(): Promise<void> {
    const cached = await this.getCachedMatches();
    if (!cached) await this.refreshCache();
  }
  ...
  async getCurrentGameWeekFixtures(): Promise<FeaturedFixtureWithImportance[]> {
    await this.ensureCache();
    const cached = await this.getCachedMatches();
    const currentWeek = this.getCurrentGameWeek(cached!);
    return cached!.filter(f => f.matchWeek === currentWeek).sort((a, b) => new Date(a.dateTime).getTime() - new Date(b.dateTime).getTime());
  }
  ...
  async getGameWeekInfo(): Promise<{
    ...
    const cached = await this.getCachedMatches();
    const currentWeek = this.getCurrentGameWeek(cached!);
    const weekFixtures = cached!.filter(f => f.matchWeek === currentWeek);
    const finishedGames = weekFixtures.filter(f => f.status === 'finished' || f.status === 'postponed').length;
    const upcomingGames = weekFixtures.filter(f => f.status === 'scheduled' || f.status === 'upcoming' || f.status === 'live').length;

    return {
      currentWeek,
      isComplete: this.isGameWeekComplete(cached!, currentWeek),
  ...
  private getCurrentGameWeek(fixtures: FeaturedFixtureWithImportance[]): number {
    ...
    for (const week of sortedWeeks) {
      ...
      const hasUnfinished = weekFixtures.some(f => ['scheduled', 'upcoming', 'live'].includes(f.status));
  ...
  private isGameWeekComplete(fixtures: FeaturedFixtureWithImportance[], gameWeek: number): boolean {
    ...
    return weekFixtures.every(f => ['finished', 'postponed'].includes(f.status));
  }
}
