...
This script trains a ZIP model to handle the 81% zero-inflation in SOT data.
ZIP models the probability of shooting separately from the count of shots.

Run after: feature_scaling.py
Outputs: zip_model.pkl, zip_training_stats.json
"""
...
def calculate_mcfadden_r2(model, y_true):
    ...
    y_mean = y_true.mean()
    ll_null = -len(y_true) * (y_mean * np.log(y_mean) - y_mean) if y_mean > 0 else 0
...
def load_data():
    ...
    try:
        ...
        if df[PREDICTOR_COLUMNS + [TARGET_COLUMN]].isnull().any().any():
            logger.warning("⚠️ NaN values detected. Dropping...")
...
def train_zip_model(X, y):
    ...
    try:
        ...
        zip_model = ZeroInflatedPoisson(
            endog=y,
            exog=X,
            exog_infl=X  # Use same features for both parts
        ).fit(method='bfgs', maxiter=1000, disp=True)
...
def evaluate_models(zip_model, poisson_model, X, y):
    ...
    if poisson_model:
        ...
        if aic_improvement > 10:
            logger.info(f"✅ DEPLOY ZIP MODEL: AIC improved by {aic_improvement:.2f} points")
        elif aic_improvement > 0:
            logger.info(f"⚠️ MARGINAL IMPROVEMENT: AIC improved by {aic_improvement:.2f} points")
            logger.info("   Consider deployment based on business requirements")
        else:
            logger.info(f"❌ KEEP POISSON: AIC worsened by {abs(aic_improvement):.2f} points")
    else:
...
def display_coefficients(zip_model):
    ...
    logger.info("─" * 80)
    logger.info(f"{'Feature':<35} {'Coefficient':<15} {'Rate Multiplier':<15}")
    logger.info("─" * 80)
    
    for i, feature in enumerate(['const'] + PREDICTOR_COLUMNS):
        coef = zip_model.params[i]
        rate_mult = np.exp(coef)
        logger.info(f"{feature:<35} {coef:<15.4f} {rate_mult:<15.4f}")
    
    # Inflation (logistic) part coefficients
    logger.info("\n🎯 INFLATION MODEL (Logistic Part) - Odds Ratios:")
    logger.info("   (Predicts probability of being a 'never-shooter')")
    logger.info("─" * 80)
    logger.info(f"{'Feature':<35} {'Coefficient':<15} {'Odds Ratio':<15}")
    logger.info("─" * 80)
    
    # Inflation parameters start after count parameters
    n_params = len(PREDICTOR_COLUMNS) + 1
    for i, feature in enumerate(['const'] + PREDICTOR_COLUMNS):
        coef = zip_model.params[n_params + i]
        odds_ratio = np.exp(coef)
        logger.info(f"{feature:<35} {coef:<15.4f} {odds_ratio:<15.4f}")
    
    logger.info("\n💡 Interpretation:")
    logger.info("  • Rate Multiplier > 1: Feature increases SOT count")
    logger.info("  • Rate Multiplier < 1: Feature decreases SOT count")
    logger.info("  • Odds Ratio > 1: Feature increases chance of being a never-shooter")
    logger.info("  • Odds Ratio < 1: Feature decreases chance of being a never-shooter")
...
def main():
    ...
    logger.info("\n🚀 Next steps:")
    logger.info("  1. Review model comparison results above")
    logger.info("  2. If ZIP is better, update live_predictor.py to use zip_model.pkl")
    logger.info("  3. Run live prediction service to test")
